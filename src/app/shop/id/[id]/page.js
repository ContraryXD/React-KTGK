         ...
         getProductById(id)
            .then((productData) => {
               ...
               if (productData && productData.category) {
                  getProductsByCategory({ category: productData.category, limit: 4 })
                     .then((relatedData) => {
                        const filteredRelated = relatedData.products.filter((p) => p.id !== productData.id);
                        setSanPhamLienQuan(filteredRelated);
                     })
                     .catch((error) => {
            ...
            .catch((error) => {
               ...
               setSanPham(null); // Set product to null if not found
            })
            .finally(() => {
               setDangTai(false);
            });
   ...
   const xuLyThemVaoGio = async () => {
      ...
      try {
         ...
         if (window.addToCart) {
            ...
            setAddToCartMessage({
               type: "success",
               text: `Successfully added ${soLuong} of ${sanPham.title} to cart!`
            });
         } else {
            ...
            setAddToCartMessage({
               type: "success",
               text: `Product "${sanPham.title}" (Qty: ${soLuong}) would be added to cart!`
            });
         }
      } catch (error) {
                     ...
                     <div className="product__details__pic">
                        <div className="product__details__pic__item">
                           <img
                              ...
                              src={sanPham.thumbnail || "/img/Image-not-found.png"}
                              alt={sanPham.title}
                           />
                        </div>
                  ...
                  <div className="col-lg-6 col-md-6">
                     <div className="product__details__text">
                        ...
                        <button
                           ...
                           disabled={isAddingToCart || sanPham.stock === 0}>
                        ...
                        {addToCartMessage && (
                           <div
                              style={{
                                 marginTop: "10px",
                                 color: addToCartMessage.type === "success" ? "green" : "red"
                              }}>
                        ...
                        <ul>
                           <li>
                              <b>Tình trạng</b>
                              <span className={sanPham.stock > 0 ? "text-success" : "text-danger"}>
                                 {sanPham.stock > 0 ? `Còn ${sanPham.stock} sản phẩm` : "Hết hàng"}
                              </span>
                           </li>
                           <li>
                     ...
                     <div className="product__details__tab">
                        <ul className="nav nav-tabs" role="tablist">
                           <li className="nav-item">
                              <a
                                 className={`nav-link ${tabHienTai === "description" ? "active" : ""}`}
                                 onClick={() => setTabHienTai("description")}
                                 role="tab">
                                 Mô tả
                              </a>
                           </li>
                           <li className="nav-item">
                              <a
                                 className={`nav-link ${tabHienTai === "information" ? "active" : ""}`}
                                 onClick={() => setTabHienTai("information")}
                                 role="tab">
                                 Thông tin
                              </a>
                           </li>
                           <li className="nav-item">
                              <a
                                 className={`nav-link ${tabHienTai === "reviews" ? "active" : ""}`}
                                 onClick={() => setTabHienTai("reviews")}
                                 role="tab">
                                    ...
                                    <table className="table">
                                       <tbody>
                                          ...
                                          <tr>
                                             ...
                                             <td>
                                                {sanPham.dimensions
                                                   ? `${sanPham.dimensions.width} x ${sanPham.dimensions.height} x ${sanPham.dimensions.depth} cm`
                                                   : "Không có"}
                                             </td>
                                          </tr>
                                       </tbody>
                                       ...
                                       sanPham.reviews.map((danhGia, index) => (
                                          <div key={index} className="mb-3 p-3 border-bottom">
                                             ...
                                             <small className="text-muted">
                                                {new Date(danhGia.date).toLocaleDateString("vi-VN")}
                                             </small>
                                          </div>
                                       ))
                                    ) : (
                        ...
                        <div key={sp.id} className="col-lg-3 col-md-4 col-sm-6">
                           <div className="product__item">
                              <div
                                 ...
                                 style={{ backgroundImage: `url(${sp.thumbnail})` }}>
